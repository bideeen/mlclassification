...
def process_folder(source_folder, dest_topfolder):
    """Processes the images in a folder"""
    for file in os.listdir(source_folder):
        if is_image(file):
            #Avoid duplicate
            if not os.path.exists(os.path.join(dest_topfolder,file)):
                #copy
                shutil.copy(os.path.join(source_folder,file),dest_topfolder)
